...
\sys turns a physical document---a bound book, but equally a stack of loose
paperwork such as intake forms or receipts---into a clean, page-separated PDF as
the operator simply flips through it. The operator points a consumer webcam at
the document like a scanning head (Figure~\ref{fig:rig}); \sys records the session
and, in real time, captures one sharp image of each spread the moment the pages
settle after a turn. Each capture is confirmed on the spot---a chime and a green
...
control loop: that live feedback lets them adjust pace and fix mistakes as they go
(Section~\ref{sec:live}). For footage shot away from a
...
live motion bar with the settle and turn thresholds marked, the current state,
a running capture count, and a green \texttt{CAPTURED} badge that flashes on each
commit---is drawn in a band \emph{above} the frame so that it
