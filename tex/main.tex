...
\begin{figure*}[t!]
    ...
    \caption{The \sys pipeline. A fixed overhead camera films an operator
    turning pages; an online state machine tracks inter-frame motion and
    auto-captures the sharpest frame each time the book settles after a turn
    (\texttt{make live}). The resulting keyframes pass through an interactive
    review stage, where the operator corrects the keyframe set and per-spread
    split geometry, before automated crop, split, and PDF assembly
    (\texttt{make finish}). The orange stages run in real time during live
    capture and produce the same artifacts as the offline front end (P1--P3),
    so a pre-recorded video can enter the green back half directly.}
    \label{fig:pipeline}
\end{figure*}
