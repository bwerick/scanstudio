...
def apply_tint(frame_bgr: np.ndarray, color=(0, 255, 0), strength: float = 0.25) -> np.ndarray:
...
class SpreadDetector:
    ...
    def __init__(self, fps=30,
                 ...
                 buffer_size=10):         # rolling buffer for smoothing
        self.fps = fps
...
def post_recording_analysis(video_path: str):
    ...
    print(f"Video: {orig_w}x{orig_h} @ {fps:.1f}fps, {total_frames} frames ({duration:.1f}s)")
    ...
    print(f"  Short spreads (<1.5s): {short_spreads}  (possible corrections/duplicates)")
    print(f"  Long spreads (>4.0s):  {long_spreads}  (possible missed turns)")
    print(f"  Motion range:         {diffs.min():.1f} — {diffs.max():.1f}")
    print(f"{'='*50}")

    if short_spreads > 0:
        print(f"  ⚠  {short_spreads} short spreads detected — you may have page-turn corrections")
    if long_spreads > 3:
    ...
    while True:
        ...
        if key in (ord("q"), 27):
            ...
            if recording:
                ...
                print(f"Recording stopped. Frames: {frames_written}, "
            ...
            snap_path = out_dir / f"snapshot_{time.strftime('%Y%m%d_%H%M%S')}_{snapshot_count:03d}.png"
